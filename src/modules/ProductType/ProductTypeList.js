...
function ProductTypeList(props) {
    ...
    const onFinishModal = async (values) => {
        ...
        if (action === ACTION_EDIT) {
            const res = await productTypeApi.editProductTypeById(editId, formData);
            if (res.status === 200) {
                notification.success({ message: t('label.edit_success')});
            }
        } else {
            const res = await productTypeApi.createProductType(formData);
            if (res.status === 200) {
    ...
    return (
        <>
            ...
            >
                {t('product_type.label.add_new_product_type')}
            </Button>
            <Modal
                footer={null}
                title={action === ACTION_ADD ? t('product_type.label.add_new_product_type') : t('product_type.label.edit_product_type')}
